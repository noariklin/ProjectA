...
class Segmenter:
    ...
    def makePredictions(self):
        ...
        basePath = os.path.join(self.predictionsBasePath, "best".format(id, epoch))
    ...
    def loadFromDisk(self, id, epoch):
        ...
        for k,v in checkpoint["net_state_dict"].items():
            if k.startswith('module.'):
                k = k[7:]
            fix_dict[k] = v
        checkpoint["net_state_dict"] = OrderedDict(fix_dict)        
    ...
    def _getCheckpointPathLoad(self, id, epoch):
        return self.checkpointsBasePathLoad + "{}/best.pt".format(id, epoch)
